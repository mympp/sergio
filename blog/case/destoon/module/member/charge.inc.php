...
switch($action) {
	case 'record':
		...
		$r = $db->get_one("SELECT COUNT(*) AS num FROM {$DT_PRE}finance_charge WHERE $condition AND reason != 'upgrade'");
		$pages = pages($r['num'], $page, $pagesize);		
		$charges = array();
		$amount = $fee = $money = $repay = 0;
		$result = $db->query("SELECT * FROM {$DT_PRE}finance_charge WHERE $condition AND reason != 'upgrade' ORDER BY itemid DESC LIMIT $offset,$pagesize");
