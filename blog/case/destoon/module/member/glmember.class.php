...
class member {
	...
	function is_username($username) {
		global $MOD, $L, $EXT, $dtedituesr;
		if($EXT['gl_jianhua_type'] && $dtedituesr){
			if(!is_mobile($username)) return $this->_($L['member_mobile_null']);
		}
	...
	function is_member($member) {
		...
		if($member['company'] && $this->company_exists($member['company'])) return $this->_($L['member_company_reg']);
		if($groupid > 5) {
			if($dtedit){
