...
if($submit && $could_up) {
	...
	if($_userid) {
		...
		$business_licence=$user['business_licence'];
		$areaid=$user['areaid'];
	}
	//地区
	$area=$db->query("SELECT * FROM {$DT_PRE}area WHERE parentid='0'");
	$area_data=array();
    while ($area_res=$db->fetch_array($area)) {
    	$area_data[]=$area_res;
    }
    if($areaid){
    //所在城市
	    $area=$db->get_one("SELECT * FROM {$DT_PRE}area WHERE areaid='$areaid'");
	    $parentid=$area['parentid'];
	    $area=$db->query("SELECT * FROM {$DT_PRE}area WHERE parentid='$parentid'");
		$area_city=array();
	    while ($area_res=$db->fetch_array($area)) {
	    	$area_city[]=$area_res;
	    }
	...
	include template('grade_my', $module);
}
?>