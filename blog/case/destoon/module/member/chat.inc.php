...
switch($action) {
	...
	case 'list':
		...
		while($r = $db->fetch_array($result)) {
			...
			$data .= '<a href="?chatid='.$r['chatid'].'" target="chat_'.$r['chatid'].'"><table cellpadding="0" cellspacing="0"><tr><td width="60">';
			$data .= '<img src="'.useravatar($r['user']).'" width="48"'.($r['online'] ? '' : ' class="chat_offline"').'/>';
			$data .= '</td><td><ul>';
			$data .= '<li><span>'.$r['last'].'</span>'.$r['user'].'</li>';
			$data .= '<li>'.($r['new'] ? '<em>'.$r['new'].'</em>' : '').($r['online'] ? $L['chat_online'] : $L['chat_offline']).' '.$r['lastmsg'].'</li>';
			$data .= '</ul></td></tr></table></a>';
		}
		if($new != $_chat) {
