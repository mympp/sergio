...
class buy {
	...
	function update($itemid) {
		...
		$update = '';
		$categorys = strip_tags(cat_pos(get_cat($item['catid']), ','));
		$pos = strpos($item['keyword'],$categorys);
		if($pos === false){
			$keyword = $item['keyword'].','.($item['tag'] ? $item['tag'].',' : '').$TYPE[$item['typeid']].','.$categorys;
		}else{
