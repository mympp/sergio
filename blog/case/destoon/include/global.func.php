...
function check_name($username) {
	...
	if (preg_match("/^[a-z0-9]{1}[a-z0-9_\-]{0,}[a-z0-9]{1}$/", $username)){
		return true;
	}else if(preg_match("/^1[3|4|5|7|8]{1}[0-9]{9}$/", $username)){
		return true;
	}else if (preg_match("/^[a-z0-9-_.]+@[\da-z][\.\w-]+\.[a-z]{2,4}$/i", $username)){
		return true;
	}else{
		return false;
	}
