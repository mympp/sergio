...
mobile_login();
$MG['mall_limit'] > -1 or dlayerm(lang('message->without_permission_and_upgrade'), 'goback');
...
switch($action) {
	case 'add':
		...
		if($MG['mall_limit'] && $limit_used >= $MG['mall_limit']) dlayerm(lang($L['info_limit'], array($MG[$MOD['module'].'_limit'], $limit_used)),'goback');
		if($MG['day_limit']) {
		...
		$thumbs= explode(",", $thumbs);
		 ...
		 $post['thumb2'] = $thumbs[2];
           ...
           $post['step']['a1'] = $post['a1'];
		   $post['step']['p1'] = $post['p1'];
		   $post['step']['a2'] = $post['a2'];
		   $post['step']['p2'] = $post['p2'];
		   $post['step']['a3'] = $post['a3'];
		   $post['step']['p3'] = $post['p3'];
			if($do->pass($post)) {
	...
	case 'refresh':
		...
		$itemid or dtexit($L['select_goods']);
		$itemids = $itemid;
		$s = $f = 0;
		foreach($itemids as $itemid) {
			$do->itemid = $itemid;
