<?php 
require 'myuser.php';
$MOD['alertid'] or mobmsg($L['feature_close']);
$MG['alert_limit'] > -1 or dlayerm(lang('mobmsg->without_permission_and_upgrade'), 'goback');
$mids = array();
...
switch($action) {
	case 'add':
		if($MG['alert_limit']) {
			...
			if($r['num'] >= $MG['alert_limit']) dlayerm(lang($L['limit_add'], array($MG['alert_limit'], $r['num'])), 'goback');
		}
		if($submit) {
			if($do->pass($post)) {
...
$foot = '';
include template('malert', $glmobpath);
