...
$updir = 'file/upload/'.timetodate($DT_TIME, $DT['uploaddir']).'/';
is_dir(DT_ROOT.'/'.$updir) or dir_create(DT_ROOT.'/'.$updir);
...
// 最终将d写入文件即可
//file_put_contents($target, $img_decode);
};


$ch = curl_init($url); // 初始化
$fp = fopen($target, 'wb'); // 打开写入
...
//exit('{"status":1,"content":"'.$imgsize.'","url":"'.$target.'"}');
