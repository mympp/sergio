<?php 
require 'myuser.php';
$MG['homepage'] && $MG['link_limit'] > -1 or dlayerm(lang('message->without_permission_and_upgrade'), 'goback');
	...
	case 'edit':
		...
		if($submit) {
			...
			if($do->pass($post)) {
				...
				$do->edit($post);
				dmsg($L['op_edit_success'], 'mlink.php?status='.$post['status']);
			} else {
				message($do->errmsg);
			}
		} else {
			extract($r);
			$head_name = $L['link_title_edit'];
		}
	break;
	case 'delete':
...
$foot = '';
include template('mlink', $glmobpath);
