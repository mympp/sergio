...
function get_reason_url($r = array()) {
	...
	switch($arr[0]) {
		...
		break;
		case 'trade':
			if(is_numeric($arr[1])) $url = 'mtrade.php?action=update&step=pay&itemid='.intval($arr[1]).'&auth='.encrypt($r['amount'], DT_KEY.'CG', 600);
		break;
		case 'trades':
			$url = 'mtrade.php?action=muti&step=pay&auth='.encrypt($arr[1], DT_KEY.'CG', 600);
		break;
		case 'group':
			if(is_numeric($arr[1])) $url = 'mgroup.php?action=update&step=pay&itemid='.intval($arr[1]).'&auth='.encrypt($r['amount'], DT_KEY.'CG', 600);
		break;
		default:
		break;
	}
...
switch($action) {
	case 'record':
		...
		$head_name = $L['charge_title_record'];
	break;
	case 'card':
		if($submit) {
			if(!preg_match("/^[0-9a-zA-z]{6,}$/", $number)) mobmsg($L['charge_pass_card_number']);
			if(!preg_match("/^[0-9]{6,}$/", $password)) mobmsg($L['charge_pass_card_password']);
			$card = $db->get_one("SELECT * FROM {$DT_PRE}finance_card WHERE number='$number'");
			if($card) {
				if($card['updatetime']) mobmsg($L['charge_pass_card_used']);
				if($card['totime'] < $DT_TIME) mobmsg($L['charge_pass_card_expired']);
				if($card['password'] != $password) mobmsg($L['charge_pass_card_error_password']);
				...
				mobmsg($L['charge_msg_card_success'], '?action=record');
			} else {
				mobmsg($L['charge_pass_card_error_number']);
			}
		}
		$head_name = $M['charge_title_card'];
	break;
	case 'repay':
		...
		$bank = $r['bank'];
		$orderid = $itemid;
		$receive_url = $glmobileurl.'mcharge.php';//$MOD['linkurl'].'charge.php';
	...
	case 'confirm':
		...
		if($MOD['mincharge']) {
			if(strpos($MOD['mincharge'], '|') !== false) {
				in_array($amount, explode('|', $MOD['mincharge'])) or mobmsg($L['charge_pass_choose_amount']);
			} else {
				$amount >= intval($MOD['mincharge']) or mobmsg($L['charge_pass_amount_min'].$MOD['mincharge']);
			}
		} else {			
			$amount > 0 or mobmsg($L['charge_pass_type_amount']);
		}
		//$amount = '0.1';
		isset($PAY[$bank]) or mobmsg($L['charge_pass_bank']);
		$PAY[$bank]['enable'] or mobmsg($L['charge_pass_bank_close']);
		...
		if($auto) $goto = 1;
		if(isset($goto)) {
			$receive_url = $glmobileurl.'mcharge.php';
			...
			$head_name = $L['charge_title_confirm'];
		}
	break;
	case 'pay':
		...
		$head_name =$M['mobile_trade_pay'];
		$barname = 1;
		}else{
		    $head_name = get_reason($reason);
		}
		}else{
			$head_name = $L['charge_title_pay'];
		}
	break;
	default:
		$_POST = $_DPOST;
		$_GET = $_DGET;
		$head_name = $L['charge_title'];
		//$passed = true;
		$charge_errcode = '';
		$charge_status = 0;
		$charge_forward = '';
		...
		if($r) {
			...
			if($r['status'] == 0) {
				...
				if($charge_status == 1) {
					...
					if($bank == 'tenpay') {
						$show_url = $charge_forward ? $charge_forward : 'mcharge.php';
...
$foot = '';
include template('mcharge', 'mobile');
