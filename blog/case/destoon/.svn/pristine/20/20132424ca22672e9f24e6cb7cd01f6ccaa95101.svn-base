...
class member {
	...
	function add($member) {
		...
		$member['sound'] = 1;
		$member_fields = array('username','company','passport', 'password','payword','email','sound','gender','truename','mobile','msn','qq','ali','skype','department','career','groupid','regid','areaid','edittime','inviter','passsalt', 'paysalt','m_user_id');
		...
		//插入推荐码，如果有的话
		$code=$member['m_user_id'];
		if($code){
			//查询search是否存在推荐码
			$search=QS_DB_NAME;
			$search_data=$this->db->query("SELECT * FROM {$search}.m_user WHERE m_user_code='$code'");	
			while($rs = $this->db->fetch_array($search_data)) {
				$m_user_id=$rs['m_user_id'];
			}
			if($m_user_id){
				$member['m_user_id']=$m_user_id;
			}
		}
	...
	function login($login_username, $login_password, $login_cookietime = 0, $admin = false) {
		...
		/*$user['m_customer_id'] = $user['userid'];
		$user['account'] = $user['username'];
		$session = new dsession();
		$_SESSION['system_data'] = $user;//echo QUESOU_PASSPORT_URL."&sid=".session_id();exit;
		file_get_contents(QUESOU_PASSPORT_URL."&sid=".session_id());*/
