...
if($action == 'update') {
	...
	$td or mobmsg($L['group_msg_null']);
	if($td['buyer'] != $_username && $td['seller'] != $_username) mobmsg($L['group_msg_deny']);
	...
	switch($step) {
		...
		case 'used':
			if($td['seller'] == $_username) {
				...
				if($td['status'] != 2 || $td['logistic']) mobmsg();
				//交易成功
				$money = $td['amount'];
		...
		break;
		case 'receive':
			if($td['status'] != 1 || $td['buyer'] != $_username || !$td['logistic']) mobmsg();
			//交易成功
			$money = $td['amount'];
			money_add($td['seller'], $money);
			money_record($td['seller'], $money, $L['in_site'], 'system', $L['group_record_pay'], $L['group_order_id'].$itemid);
		...
		break;
		case 'send':
			if($td['status'] != 0 || $td['seller'] != $_username || !$td['logistic']) mobmsg();
			if($submit) {
				...
				$head_name = $L['group_send_title'];
				$send_types = explode('|', trim($MOD['send_types']));
				$send_time = timetodate($DT_TIME, 3);
			}
		break;
		case 'add_time'://增加确认收货时间
			if(!in_array($td['status'], array(1, 2)) || $td['seller'] != $_username) mobmsg();
			if($submit) {
				$add_time = intval($add_time);
				$add_time > 0 or mobmsg($L['group_addtime_null']);
				$add_time = $td['add_time'] + $add_time;
				$db->query("UPDATE {$table} SET add_time='$add_time' WHERE itemid=$itemid");
				mobmsg($L['group_addtime_success'], $forward);
			} else {
				$head_name = $L['group_addtime_title'];
			}
		break;
		case 'print'://订单打印
			if($td['seller'] != $_username || !$td['logistic']) mobmsg($L['group_msg_deny']);
			$td['total'] = $td['amount'];
		...
		case 'pay'://买家付款
			...
			if($td['status'] != 6 || $td['buyer'] != $_username) mobmsg($L['group_msg_deny']);
			$money = $td['amount'];
			$money > 0 or mobmsg($L['group_msg_deny']);
			...
			if($submit) {
				...
				$_money >= $money or mobmsg($L['money_not_enough']);
				...
				if($password) {
					//send sms
					if($DT['sms']) {
						$mobmsg = lang('sms->ord_group', array($td['title'], $itemid, $password));
						$mobmsg = strip_sms($mobmsg);
						send_sms($td['buyer_mobile'], $mobmsg);
					}
					//send sms
				}
				...
				$head_name = $L['group_pay_order_title'];
			}
		break;
		case 'refund'://买家退款
			$gone = $DT_TIME - $td['updatetime'];
			if(!in_array($td['status'], array(0, 1, 2)) || $td['buyer'] != $_username) mobmsg($L['group_msg_deny']);
			if(in_array($td['status'], array(1, 2)) && $gone > ($MOD['trade_day']*86400 + $td['add_time']*3600)) mobmsg($L['group_msg_deny']);
			$money = $td['amount'];
			if($submit) {
		...
		case 'refund_agree'://卖家同意买家退款
			if($td['status'] != 4 || $td['seller'] != $_username) mobmsg($L['trade_msg_deny']);
			$money = $td['amount'];
			if($submit) {
		...
		case 'remind'://买家提醒卖家发货
			if($td['status'] != 0 || $td['buyer'] != $_username || !$td['logistic']) mobmsg($L['group_msg_deny']);
		break;
		case 'get_pay'://买家确认超时 卖家申请直接付款
			$gone = $DT_TIME - $td['updatetime'];
			if(!in_array($td['status'], array(1, 2)) || $td['seller'] != $_username || $gone < ($MOD['trade_day']*86400 + $td['add_time']*3600)) mobmsg($L['group_msg_deny']);
			//交易成功
			$money = $td['amount'];
			money_add($td['seller'], $money);
			...
			$db->query("UPDATE {$table} SET status=3,updatetime=$DT_TIME WHERE itemid=$itemid");
			mobmsg($L['group_success'], $forward, 3);
		break;
	}
	...
	$money = number_format($money, 2, '.', '');
	$forward = urlencode($DT_URL);
	$head_name = $L['group_title'];
}
$foot = '';
include template('mgroup', 'mobile');
