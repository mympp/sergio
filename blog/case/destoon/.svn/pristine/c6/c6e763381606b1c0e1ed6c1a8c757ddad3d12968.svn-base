...
mobile_login();
$MG['exhibit_limit'] > -1 or dlayerm(lang('message->without_permission_and_upgrade'), 'goback');
...
switch($action) {
	case 'add':
		...
		if($MG['exhibit_limit'] && $limit_used >= $MG['exhibit_limit']) dlayerm(lang($L['info_limit'], array($MG[$MOD['module'].'_limit'], $limit_used)), $_userid ? $MODULE[2]['linkurl'].$DT['file_my'].'?mid='.$mid : $MODULE[2]['linkurl'].$DT['file_my']);
		if($MG['day_limit']) {
	...
	break;
	case 'sign':
	    $head_name = $M['sign_mobile'].$M['manage_mobile'];
		$back_link = $MYURL.'?action=info';
