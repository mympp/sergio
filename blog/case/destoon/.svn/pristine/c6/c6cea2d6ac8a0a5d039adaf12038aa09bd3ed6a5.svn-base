...
switch($action) {
	...
	case 'list':
		...
		while($r = $db->fetch_array($result)) {
			...
			$data .= '<li class="aui-list-item aui-list-item-middle"><div class="aui-media-list-item-inner"><div class="aui-list-item-media" style="width: 3rem;">';
			$data .= '<a href="chat.php?chatid='.$r['chatid'].'&reload='.$DT_TIME.'"><img src="'.useravatar($r['user']).'" class="aui-img-round aui-list-img-sm"/></div>';
			$data .= '<div class="aui-list-item-inner aui-list-item-arrow"><div class="aui-list-item-text">';
			$data .= '<div class="aui-list-item-title"><strong>'.$r['user'].'</strong></div><div class="aui-list-item-right"><span>'.$r['last'].'</span></div></div>';
			$data .= '<div class="aui-list-item-text"><div class="chat-online">'.($r['new'] ? '<em>'.$r['new'].'</em>' : '').($r['online'] ? $L['chat_online'] : $L['chat_offline']).' '.$r['lastmsg'].'</div></div>';
			$data .= '</div></div></li>';
		}
		if($new != $_chat) {
