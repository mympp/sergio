...
if($moduleid==1 || $itemid) $mindex = 1;
//定位开始
if($EXT['gl_mobile_city']){
$m_history = get_cookie('mcity');
$history = explode(',', $m_history);
$cityid = ($history[0])?$history[0]:intval($gocity);
if($cityid>0){
    if($cityid && !$history[0])set_cookie('mcity', $cityid, $DT_TIME + 7*86400);
	if($mindex==1){
	$pareaid = gl_get_parareaid($cityid);
	$areaname = gl_area_name($cityid);

    if(isset($areaid) || $areaid){
    $sql="select areaid,areaname from {$DT_PRE}area where parentid='$pareaid'";
    $glarea = array();
    $res = $db->query($sql);
