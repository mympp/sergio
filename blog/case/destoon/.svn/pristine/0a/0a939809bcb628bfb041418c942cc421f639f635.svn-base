...
if($action == 'update') {
	...
	$td or mobmsg($L['trade_msg_null']);
	if($td['buyer'] != $_username && $td['seller'] != $_username) mobmsg($L['trade_msg_deny']);
	...
	switch($step) {
		case 'edit_price'://修改价格||确认订单||修改为货到付款
			if($td['status'] > 1 || $td['seller'] != $_username) mobmsg($L['trade_msg_deny']);
			if($DT['trade'] && $_trade == '') mobmsg(lang($L['trade_msg_pay_bind'], array($DT['trade_nm'])), '?action=bind');
			if($submit) {
				$fee = dround($fee);
				if($fee < 0 && $fee < -$td['amount']) mobmsg(lang($L['trade_msg_less_fee'], array(-$td['amount'])));
				...
				mobmsg($L['trade_price_edit_success'], 'mtrade.php', 3);
			} else {
				$confirm = isset($confirm) ? 1 : 0;
				$head_name = $L['trade_price_title'];
			}
		break;
		case 'detail'://订单详情
			$td['total'] = $td['amount'] + $td['fee'];
			$auth = encrypt('mall|'.$td['send_type'].'|'.$td['send_no'].'|'.$td['send_status'].'|'.$td['itemid'], DT_KEY.'EXPRESS');
			$head_name = $L['trade_detail_title'];
		break;
		case 'print'://订单打印
			$td['total'] = $td['amount'] + $td['fee'];
			if($td['seller'] != $_username) mobmsg($L['trade_msg_deny']);
		...
		case 'express'://快递追踪
			...
			$auth = encrypt('mall|'.$td['send_type'].'|'.$td['send_no'].'|'.$td['send_status'].'|'.$td['itemid'], DT_KEY.'EXPRESS');
			$head_name = $L['trade_exprss_title'];
		break;
		case 'pay'://买家付款
			if($td['status'] == 0) mobmsg($L['trade_msg_confirm'], '?action=update&step=detail&itemid='.$itemid);
			if($td['status'] != 1 || $td['buyer'] != $_username) mobmsg($L['trade_msg_deny']);
			$money = $td['amount'] + $td['fee'];
			$money > 0 or mobmsg($L['trade_msg_deny']);
			...
			if($submit) {
				if(!$auto) {
					is_payword($_username, $password) or dlayerm($L['error_payword']);
				}
				$_money >= $money or window.history.back();($L['money_not_enough']);
				...
				$head_name = $L['trade_pay_order_title'];
			}
		break;
		case 'refund'://买家退款
			if($DT['trade']) exit(include DT_ROOT.'/api/trade/'.$DT['trade'].'/update.inc.php');
			$gone = $DT_TIME - $td['updatetime'];
			if(!in_array($td['status'], array(2, 3)) || $td['buyer'] != $_username) mobmsg($L['trade_msg_deny']);
			if($td['status'] == 3 && $gone > ($MOD['trade_day']*86400 + $td['add_time']*3600)) mobmsg($L['trade_msg_deny']);
			$money = $td['amount'] + $td['fee'];
			if($submit) {
				$content or mobmsg($L['trade_refund_reason']);
				...
				mobmsg($L['trade_refund_success'], $forward, 3);
			} else {
				$head_name = $L['trade_refund_title'];
			}
		break;
		case 'refund_agree'://卖家同意买家退款
			if($DT['trade']) exit(include DT_ROOT.'/api/trade/'.$DT['trade'].'/update.inc.php');
			if($td['status'] != 5 || $td['seller'] != $_username) mobmsg($L['trade_msg_deny']);
			$money = $td['amount'] + $td['fee'];
			if($submit) {
				...
				mobmsg($L['trade_refund_agree_success'], $forward, 3);
			} else {
				$head_name = $L['trade_refund_agree_title'];
			}
		break;
		case 'remind'://买家提醒卖家发货			
			if($td['status'] != 2 || $td['buyer'] != $_username) mobmsg($L['trade_msg_deny']);
		break;
		case 'send_goods'://卖家发货
			if(($td['status'] != 2 && $td['status'] != 7) || $td['seller'] != $_username) mobmsg($L['trade_msg_deny']);
			if($DT['trade'] && $td['status'] == 2) exit(include DT_ROOT.'/api/trade/'.$DT['trade'].'/update.inc.php');
			if($submit) {
				$send_type = trim(dhtmlspecialchars($send_type));
				if(strlen($send_type) > 2 && strlen($send_no) < 5) mobmsg($L['msg_express_no']);
				if(strlen($send_no) > 4 && strlen($send_type) < 3) mobmsg($L['msg_express_type']);
				if($send_no && !preg_match("/^[a-z0-9_\-]{4,}$/i", $send_no)) mobmsg($L['msg_express_no_error']);
				is_date($send_time) or mobmsg($L['msg_express_date_error']);
				...
				mobmsg($L['trade_send_success'], $forward, 3);
			} else {
				$head_name = $L['trade_send_title'];
				$send_types = explode('|', trim($MOD['send_types']));
				$send_time = timetodate($DT_TIME, 3);
			}
		break;
		case 'cod_success'://货到付款，确认完成
			if($td['status'] != 7 || !$td['cod'] || !$td['send_time'] || $td['seller'] != $_username) mobmsg($L['trade_msg_deny']);
			$db->query("UPDATE {$table} SET status=4,updatetime=$DT_TIME WHERE itemid=$itemid");
			//交易成功
			mobmsg($L['trade_success'], $forward, 3);
			
		break;
		case 'add_time'://增加确认收货时间
			if($DT['trade']) exit(include DT_ROOT.'/api/trade/'.$DT['trade'].'/update.inc.php');
			if($td['status'] != 3 || $td['seller'] != $_username) mobmsg($L['trade_msg_deny']);
			if($submit) {
				$add_time = intval($add_time);
				$add_time > 0 or mobmsg($L['trade_addtime_null']);
				$add_time = $td['add_time'] + $add_time;
				$db->query("UPDATE {$table} SET add_time='$add_time' WHERE itemid=$itemid");
				mobmsg($L['trade_addtime_success'], $forward);
			} else {
				$head_name = $L['trade_addtime_title'];
			}
		break;
		case 'receive_goods'://确认收货
			if($DT['trade']) exit(include DT_ROOT.'/api/trade/'.$DT['trade'].'/update.inc.php');
			$gone = $DT_TIME - $td['updatetime'];
			if($td['status'] != 3 || $td['buyer'] != $_username || $gone > ($MOD['trade_day']*86400 + $td['add_time']*3600)) mobmsg($L['trade_msg_deny']);
			//交易成功
			$money = $td['amount'] + $td['fee'];
		...
		case 'get_pay'://买家确认超时 卖家申请直接付款
			if($DT['trade']) exit(include DT_ROOT.'/api/trade/'.$DT['trade'].'/update.inc.php');
			$gone = $DT_TIME - $td['updatetime'];
			if($td['status'] != 3 || $td['seller'] != $_username || $gone < ($MOD['trade_day']*86400 + $td['add_time']*3600)) mobmsg($L['trade_msg_deny']);
			//交易成功
			$money = $td['amount'] + $td['fee'];
			money_add($td['seller'], $money);
		...
		break;
		case 'comment'://交易评价
			if($td['mid'] != 16) mobmsg($L['trade_msg_deny_comment']);
			if($submit) {
				$star = intval($star);
			...
			if($_username == $td['seller']) {
				if($td['buyer_star']) mobmsg($L['trade_msg_comment_again']);
				if($submit) {
					...
					mobmsg($L['trade_msg_comment_success'], $forward);
				}
			} else if($_username == $td['buyer']) {
				if($td['seller_star']) mobmsg($L['trade_msg_comment_again']);
				if($submit) {
		...
		case 'comment_detail'://评价详情
		    $head_name = $M['comment_detail'];
			if($td['mid'] != 16) mobmsg($L['trade_msg_deny_comment']);
			$cm = $db->get_one("SELECT * FROM {$DT_PRE}mall_comment WHERE itemid=$itemid");
			if($submit) {
				$content = dhtmlspecialchars($content);
				$content or mobmsg($L['trade_msg_empty_explain']);
				if($_username == $td['seller']) {
					if($cm['buyer_reply']) mobmsg($L['trade_msg_explain_again']);
					$db->query("UPDATE {$DT_PRE}mall_comment SET buyer_reply='$content',buyer_rtime=$DT_TIME WHERE itemid=$itemid");
				} else {
					if($cm['seller_reply']) mobmsg($L['trade_msg_explain_again']);
		...
		case 'close'://关闭交易
			if($_username == $td['seller']) {
				if($td['status'] == 0) {
					...
					mobmsg($L['trade_msg_deny']);
				}
				mobmsg($L['trade_close_success'], $forward);
			} else if($_username == $td['buyer']) {
				if($td['status'] == 0) {
	...
	$DT['trade'] or mobmsg($L['trade_msg_secured_close']);
	$member = $db->get_one("SELECT trade,vtrade FROM {$DT_PRE}member WHERE userid=$_userid");
	if($submit) {
		if($member['trade'] && $member['vtrade']) mobmsg($L['trade_msg_bind_edit']);
		if($trade) {
			if($DT['trade'] == 'alipay' && !is_email($trade) && !is_mobile($trade)) mobmsg(lang($L['trade_bind_error'], array($DT['trade_nm'])));
			$r = $db->get_one("SELECT userid FROM {$DT_PRE}member WHERE trade='$trade' AND vtrade=1");
			if($r) mobmsg($L['trade_msg_bind_exists']);
		} else {
			$trade = '';
		}
		...
		$head_name = lang($L['trade_bind_title'], array($DT['trade_nm']));
	}
} else if($action == 'muti') {//批量付款
	if($submit) {
		is_payword($_username, $password) or mobmsg($L['error_payword']);
		($itemid && is_array($itemid)) or mobmsg($L['trade_msg_muti_choose']);
		...
		$lists or mobmsg($L['trade_msg_muti_empty'], '?action=order', 5);
		$head_name = $L['trade_muti_title'];
	}
	...
	$money = $amount + $fee;
	$money = number_format($money, 2, '.', '');
	$head_name = $L['trade_title'];
}

$foot = '';
include template('mtrade', 'mobile');
