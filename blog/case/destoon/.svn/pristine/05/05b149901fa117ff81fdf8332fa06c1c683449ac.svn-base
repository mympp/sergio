...
switch($action) {
		...
		if($MOD['credit_exchange'] && $MOD['ex_host'] && $MOD['ex_user'] && $MOD['ex_data'] && $MOD['ex_type']) {
			...
			if($MOD['ex_type'] == 'PW') {
				...
				if($r) {
					...
					mobmsg($L['credit_msg_active'], '?action=index');
				}
			...
			if($submit) {
				$num > 0 or mobmsg($L['credit_pass_ex_min']);
				...
				if($amount > 0 && $amount <= $num) {
					...
					mobmsg($L['credit_pass_ex_max'].$num);
				}
			}
		} else {
			mobmsg($L['feature_close'], '?action=index');
		}
		$head_name = $L['credit_exchange_title'];
	break;
	case 'buy':
		if($MOD['credit_buy'] && $MOD['credit_price']) {
			...
			if($submit) {
				...
				array_key_exists($type, $C) or mobmsg($L['credit_msg_buy_amount']);
				$amount = $P[$type];
				$credit = $C[$type];
				if($amount > 0) {
					$_money >= $amount or mobmsg($L['money_not_enough'], 'charge.php?action=pay&reason=credit&amount='.($amount-$_money));
		...
		$head_name = $L['credit_buy_title'];
	break;
	case 'invite':
		$head_name = $L['invite_title'];
	...
	default:
		...
		$head_name = $L['credit_title'];
	break;
}
	$foot = '';
	include template('mcredit', 'mobile');
