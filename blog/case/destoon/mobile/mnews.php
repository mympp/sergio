...
$MG['homepage'] && $MG['news_limit'] > -1 or dlayerm(lang('mobmsg->without_permission_and_upgrade'), 'goback');
...
switch($action) {
	case 'add':
		if($MG['news_limit']) {
			...
			if($r['num'] >= $MG['news_limit']) dlayerm(lang($L['limit_add'], array($MG['news_limit'], $r['num'])), 'goback');
		}
		if($submit) {
			if($do->pass($post)) {
	...
	case 'edit':
		...
		if($submit) {
			...
			$head_name = $L['news_title_edit'];
		}
	break;
	case 'delete':
		$itemid or mobmsg($L['news_msg_choose']);
...
$foot = '';
include template('mnews', $glmobpath);
