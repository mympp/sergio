<?php 
require 'myuser.php';
$MG['favorite_limit'] > -1 or dlayerm(lang('message->without_permission_and_upgrade'), 'goback');
...
switch($action) {
	case 'add':
		if($MG['favorite_limit']) {
			...
			if($r['num'] >= $MG['favorite_limit']) dlayerm(lang($L['limit_add'], array($MG['favorite_limit'], $r['num'])), 'goback');
		}
		if($submit) {
			if($do->pass($post)) {
				$post['userid'] = $_userid;
			...
			$head_name = $L['favorite_title_add'];
		}
	break;
	case 'edit':
		$itemid or mobmsg();
		...
		if($submit) {
			...
			$head_name = $L['favorite_title_edit'];
		}
	break;
	case 'delete':
		$itemid or mobmsg($L['favorite_msg_choose']);
		$do->itemid = $itemid;
		$r = $do->get_one();
		if(!$r || $r['userid'] != $_userid) mobmsg();
		$do->delete($itemid);
	...
	default:
		...
		$action = 'index';
		$head_name = $L['favorite_title'];
	break;
}
$foot = '';
include template('mfavorite', $glmobpath);
