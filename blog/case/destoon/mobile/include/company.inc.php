...
if($userid || $username) {
	...
	if(in_array($action, array('mall', 'sell', 'buy', 'job', 'photo', 'info', 'brand', 'video'))) {
		...
		//include template('homepage101', 'mobile');
		//二次开发结束
		isset($_MENU[$action]) or dheader($HURL);
	...
	$mylat = isset($mylat) ? strip_tags($mylat) : $mlat;
	$mylng = isset($mylng) ? strip_tags($mylng) : $mlng;
	set_cookie('mylat', $mylat, $DT_TIME + 1800);
	set_cookie('mylng', $mylng, $DT_TIME + 1800);
	
    $km = isset($km) ? strip_tags($km) : 0;
	...
	if(get_cookie('my_addrs')){
	  $addluser = get_cookie('my_addrs');
	}else{
	  $addluser = gl_comaddress($mylat.','.$mylng);
	  set_cookie('my_addrs', $addluser, $DT_TIME + 1800);
	}
	...
	if($items) {
		...
		while($r = $db->fetch_array($result)) {
			...
			if(gl_comlng($r['userid']) && $EXT['gl_nearcom']){
			$compoint = gl_comlng($r['userid']);
			//$r['localcom'] = $compoint['1'].','.$compoint['0'];
			if($compoint)$r['Distance'] = gl_GetDistance($mylat,$mylng,$compoint['1'],$compoint['0'],2);
            if($r['address'])$r['glmap'] = "bdmap.php?star=".$addluser."&end=".$r['address'];
			}
			$lists[] = $r;
		}
