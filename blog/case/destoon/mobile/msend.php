...
switch($action) {
	case 'check':
		...
		if(!$could_email) mobmsg($L['send_mail_close']);
		...
		if($step == 2) {
			...
			(preg_match("/^[0-9a-z]{6,}$/i", $code) && $_SESSION['email_code'] == md5($email.'|'.$code.'|SEC')) or mobmsg($L['register_pass_emailcode']);
			...
			if($r) {
				if($r['groupid'] != 4) mobmsg($L['send_check_deny']);
				$emailcode = random(8);
				...
				mobmsg($L['send_bad_email']);
			}
		} else {
			(isset($username) && check_name($username)) or $username = '';
			$head_name = $L['send_email_title'];
		}
	break;
	case 'passport':
		$_username == $_passport or dheader('medit.php');
		if($submit) {
			...
			if($do->edit_passport($_passport, $npassport, $_username)) {
				dmsg($L['op_edit_success'], 'medit.php');
			} else {
				mobmsg($do->errmsg);
			}
		} else {			
			$head_name = $L['send_passport_title'];
		}
	break;
	case 'payword':
		mobile_login();
		...
		if($step == 2) {
			...
			$code = isset($code) ? trim($code) : '';
			(preg_match("/^[0-9a-z]{6,}$/i", $code) && $_SESSION['email_code'] == md5($payword.'|'.$code.'|'.$username.'|SEP')) or mobmsg($L['register_pass_emailcode']);
			$salt = random(8);
			...
			captcha($captcha);
			if(strlen($npassword) > $MOD['maxpassword'] || strlen($npassword) < $MOD['minpassword']) mobmsg(lang($L['member_password_len'], array($MOD['minpassword'], $MOD['minpassword'])));
			if($npassword != $cpassword) mobmsg($L['member_payword_match']);
			if(!is_password($username, $password)) mobmsg($L['member_login_password_bad']);
			$emailcode = random(8);
			...
			$head_name = $L['send_payword_title'];
		}
	break;
	case 'email':
		mobile_login();
		...
		if($step == 2) {
			...
			is_email($email) or dheader('?action='.$action);
			$code = isset($code) ? trim($code) : '';
			(preg_match("/^[0-9a-z]{6,}$/i", $code) && $_SESSION['email_code'] == md5($email.'|'.$code.'|'.$username.'|SE')) or mobmsg($L['register_pass_emailcode']);
			...
			captcha($captcha);
			is_email($email) or mobmsg($L['member_email_null']);
			if($email == $_email) mobmsg($L['send_email_exist']);
			if(!is_password($username, $password)) mobmsg($L['member_login_password_bad']);
	...
	case 'mobile':
		...
		if($step == 2) {
			...
			$code = isset($code) ? trim($code) : '';
			(preg_match("/^[0-9a-z]{6,}$/i", $code) && $_SESSION['mobile_code'] == md5($mobile.'|'.$code.'|'.$username.'|SM')) or mobmsg($L['register_pass_mobilecode']);
			...
			if($mobile == $_mobile) mobmsg($L['send_mobile_exist']);
			if(!is_password($username, $password)) mobmsg($L['member_login_password_bad']);
            ...
            alidayu_sms($mobile, $mobilecode,1);
			}else{
			send_sms($mobile, $content);
			}
		} else {
			$head_name = $L['send_mobile_title'];
		}
	break;
	case 'contact':
		$url = DT_PATH;
		...
		$head_name = $L['send_password_title'];
	break;
	case 'sms':
		...
		if($step == 2) {
			$mobile = $_SESSION['mobile_save'];
			is_mobile($mobile) or dheader('?action='.$action);
			if(strlen($password) > $MOD['maxpassword'] || strlen($password) < $MOD['minpassword']) mobmsg(lang($L['member_password_len'], array($MOD['minpassword'], $MOD['minpassword'])));
			if($password != $cpassword) mobmsg($L['member_payword_match']);
			$code = isset($code) ? trim($code) : '';
			(preg_match("/^[0-9a-z]{6,}$/i", $code) && $_SESSION['mobile_code'] == md5($mobile.'|'.$code.'|SMS')) or mobmsg($L['register_pass_mobilecode']);
			$r = $db->get_one("SELECT userid,username,groupid,vmobile FROM {$DT_PRE}member WHERE mobile='$mobile' AND vmobile=1");
			if($r) {
				if($r['groupid'] == 2 || $r['groupid'] == 4) mobmsg($L['send_password_checking']);
				$userid = $r['userid'];
				$username = $r['username'];
				$salt = random(8);
				...
				mobmsg($L['send_bad_mobile']);
			}
		} else if($step == 1) {
			captcha($captcha);
			if(max_sms($mobile)) mobmsg($L['sms_msg_max'], '?action='.$action);
			if($_SESSION['mobile_send'] > 4) mobmsg($L['send_too_many'], '?action='.$action);
			if($second < 180) mobmsg($L['send_too_quick'], '?action='.$action);
			$mobile = trim($mobile);
			if(!is_mobile($mobile)) mobmsg($L['member_mobile_null']);
	...
	case 'mail':
		...
		if($step == 2) {
			...
			if($password != $cpassword) mobmsg($L['member_payword_match']);
			$code = isset($code) ? trim($code) : '';
			(preg_match("/^[0-9a-z]{6,}$/i", $code) && $_SESSION['email_code'] == md5($email.'|'.$code.'|SEM')) or mobmsg($L['register_pass_emailcode']);
			...
			captcha($captcha);
			if($_SESSION['email_send'] > 9) mobmsg($L['send_too_many'], '?action='.$action);
			if($second < 60) mobmsg($L['send_too_quick'], '?action='.$action);
			$email = trim($email);
			is_email($email) or mobmsg($L['member_email_null']);
			$r = $db->get_one("SELECT groupid FROM {$DT_PRE}member WHERE email='$email'");
			if($r) {
