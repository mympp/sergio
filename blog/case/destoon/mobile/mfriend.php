<?php 
require 'myuser.php';
$MG['friend_limit'] > -1 or dlayerm(lang('message->without_permission_and_upgrade'), 'goback');
...
switch($action) {
	case 'add':
		if($MG['friend_limit']) {
			...
			if($r['num'] >= $MG['friend_limit']) dlayerm(lang($L['limit_add'], array($MG['friend_limit'], $r['num'])), 'goback');
		}
		if($submit) {
			if($do->pass($post)) {
				if($post['username'] && $db->get_one("SELECT username FROM {$DT_PRE}friend WHERE userid=$_userid AND username='$post[username]'")) mobmsg($L['friend_msg_add_again']);
				$post['userid'] = $_userid;
	...
	case 'edit':
		...
		if($submit) {
			...
			$head_name = $L['friend_title_edit'];
		}
	break;
	case 'delete':
		$itemid or mobmsg($L['friend_msg_choose']);
		$do->itemid = $itemid;
		$r = $do->get_one();
		if(!$r || $r['userid'] != $_userid) mobmsg();
		$do->delete($itemid);
...
$foot = '';
include template('mfriend', $glmobpath);
