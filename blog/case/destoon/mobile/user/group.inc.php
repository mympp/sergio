...
mobile_login();
$MG['group_limit'] > -1 or dlayerm(lang('message->without_permission_and_upgrade'), 'goback');
...
switch($action) {
	case 'add':
		...
		if($MG['group_limit'] && $limit_used >= $MG['group_limit']) dlayerm(lang($L['info_limit'], array($MG[$MOD['module'].'_limit'], $limit_used)), 'goback');
		if($MG['day_limit']) {
		...
		$thumbs= explode(",", $thumbs);
		 ...
		 $post['thumb2'] = $thumbs[2];
			...
			if($do->pass($post)) {
				...
				if($MOD['upload_thumb'] && $_userid && strlen($post['thumb']) < 5) dtexit($L['group_upload_image']);
