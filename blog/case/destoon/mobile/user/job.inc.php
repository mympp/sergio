...
if($resume) {
$MG['resume_limit'] > -1 or dlayerm(lang('message->without_permission_and_upgrade'), 'goback');
...
switch($action) {
	case 'add':
	    $head_name = $L['resume_name'].$M['add_mobile'];
		$back_link = $MUSER.'?mid='.$mid;
		if($MG['resume_limit'] && $limit_used >= $MG['resume_limit']) dlayerm(lang($L['info_limit'], array($MG[$MOD['module'].'_limit'], $limit_used)),'goback');
		if($MG['day_limit']) {
		 ...
		 $post['thumb2'] = $thumbs[2];
			...
			if($do->pass($post)) {
				...
				if($_userid) {
					...
					dtexit('ok','?mid='.$mid.'&resume=1&status='.$post['status'].'',''.$msg.'');
					dtexit('', '', 'parent.window.location="'.$forward.'";');
				} else {
					dtexit('ok','?mid='.$mid.'&resume=1&status='.$post['status'].'',''.$msg.'');
					glerror($msg, '', 'parent.window.location=parent.window.location;');
				}
			} else {
	...
	break;
	case 'edit':
	    $head_name = $L['resume_name'].$M['edit_mobile'];
		$back_link = $MYURL.'?action=info';
		$itemid or dlayerm($M['other_error']);
		$do->itemid = $itemid;
		$item = $do->get_one();
		if(!$item || $item['username'] != $_username) dlayerm($M['other_error']);
		if(isset($_POST['ok'])) {
		$thumbs= explode(",", $thumbs);
		 $post['thumb'] = $thumbs[0];
		 $post['thumb1'] = $thumbs[1];
		 $post['thumb2'] = $thumbs[2];
			if($do->pass($post)) {
				...
				if($FD) fields_update($post_fields, $table, $do->itemid);
				$do->edit($post);
                dtexit('ok','?mid='.$mid.'&resume=1&status='.$post['status'].'');
	...
	break;
	case 'apply':
	    $head_name = $L['resume_name'].$M['manage_mobile'];
		$condition = '';
	...
	case 'refresh':
		...
		if(!$item || $item['username'] != $_username) dtexit($L['select_info']);
		if($MG['refresh_limit'] && $DT_TIME - $item['edittime'] < $MG['refresh_limit']) dtexit(lang($L['refresh_limit'], array($MG['refresh_limit'])), $forward);
		$do->refresh($itemid);
		dtexit('ok');
		dmsg($L['success_update'], $forward);
	break;
...
include template('my_resume', 'mobile');

} else {

$MG['job_limit'] > -1 or dlayerm(lang('message->without_permission_and_upgrade'), 'goback');
...
switch($action) {
	case 'add':
	    $head_name = $L['job_name'].$M['add_mobile'];
		$back_link = $MUSER.'?mid='.$mid;
		if($MG['job_limit'] && $limit_used >= $MG['job_limit']) dlayerm(lang($L['info_limit'], array($MG[$MOD['module'].'_limit'], $limit_used)),'goback');
		if($MG['day_limit']) {
			$today = $today_endtime - 86400;
			$r = $db->get_one("SELECT COUNT(*) AS num FROM {$table} WHERE $sql AND addtime>$today");
			if($r && $r['num'] >= $MG['day_limit']) dlayerm(lang($L['day_limit'], array($MG['day_limit'])),'goback');
		...
		if(isset($_POST['ok'])) {
			...
			if($itemid) {
				$MG['copy'] or dlayerm(lang('message->without_permission_and_upgrade'), 'goback');
				$do->itemid = $itemid;
				$item = $do->get_one();
	...
	break;
	case 'edit':
	    $head_name = $L['job_name'].$M['edit_mobile'];
		$back_link = $MUSER.'?mid='.$mid;
		$itemid or dlayerm();
		$do->itemid = $itemid;
		$item = $do->get_one();
	...
	case 'resume_invite':
		...
		$apply or mobmsg($L['msg_not_apply']);
		$resume = $db->get_one("SELECT * FROM {$DT_PRE}resume WHERE itemid='$apply[resumeid]'");
		$resume or mobmsg($L['msg_not_resume']);
		if(!$resume['username']) mobmsg($L['msg_not_member']);
		if($resume['status'] != 3) mobmsg($L['msg_resume_close']);
		$job = $db->get_one("SELECT * FROM {$DT_PRE}job WHERE itemid='$apply[jobid]' AND status=3");
		$job or mobmsg($L['msg_not_job']);
		if($job['totime'] && $job['totime'] < $DT_TIME) mobmsg($L['msg_job_expired']);
	...
	break;
	default:
	    $head_name = $L['job_name'].$M['manage_mobile'];
		$back_link = $MYURL.'?action=info';
		$status = isset($status) ? intval($status) : 3;
		in_array($status, array(1, 2, 3, 4)) or $status = 3;
		$condition = "username='$_username'";
