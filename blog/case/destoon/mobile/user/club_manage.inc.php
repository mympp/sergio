...
if($action) {
	...
	if(!is_admin($GRP)) dlayerm($L['my_not_admin']);
	...
	if($MOD['manage_reason']) {
		...
		if($need_reason && ($reason == $L['my_manage_reason'] || strlen($reason) < 4)) dlayerm($L['my_manage_input_reason']);
	}
}
switch($action) {
	case 'edit':
	    $head_name = $M['edit_mobile'].$L['my_manage_type_post'];
		$itemid or dlayerm('no itemid');
		require MD_ROOT.'/club.class.php';
		$do = new club($moduleid);
		$do->itemid = $itemid;
		$T = $do->get_one();
		if(!$T || $T['gid'] != $gid || $T['status'] != 3) dlayerm($L['my_not_post']);		
		if(isset($_POST['ok'])) {
			foreach($post as $k=>$v) {
			...
			if($do->pass($post)) {		
				...
				dtexit('ok',''.$forward.'',''.$msg.'');
				dtexit($L['post_success_edit'], $forward);
			} else {
				dtexit($do->errmsg);
			}
		} else {
	...
	break; 
	case 'delete':
		($itemid && is_array($itemid)) or dlayerm($L['my_choose_post']);
		$content = '';
		if($reason == $L['my_manage_reason']) $reason = '';
		$reason = dhtmlspecialchars($reason);
		$message = isset($message) ? 1 : 0;
	...
	case 'level':
		...
		in_array($level, array(0, 1, 2, 3)) or dlayerm($L['my_manage_not_level']);
		$content = $level;
		if($reason == $L['my_manage_reason']) $reason = '';
		$reason = dhtmlspecialchars($reason);
		$message = isset($message) ? 1 : 0;
	...
	break;
	case 'post':
	    $head_name = '['.$GRP['title'].']'.$L['my_manage_post'];
	...
	break;
	case 'reply':
		$head_name = '['.$GRP['title'].']'.$L['my_manage_reply'];
		$back_link = $MYURL.'?action=info';
	...
	case 'reply_edit':
		...
		$do->itemid = $itemid;
		$R = $do->get_one();
		if(!$R || $R['gid'] != $gid || $R['status'] != 3) dlayerm($L['my_not_reply']);		
		if(isset($_POST['ok'])) {
			foreach($post as $k=>$v) {
				$post[$k] = convert(input_trim($v), 'UTF-8', DT_CHARSET);
			}
			...
			if($do->pass($post)) {		
				...
				dtexit('ok',''.$forward.'',''.$msg.'');
				dtexit($L['reply_success_edit'], $forward);
			} else {
				dtexit($do->errmsg);
			}
		} else {
	...
	break; 
	case 'reply_delete':
		($itemid && is_array($itemid)) or dlayerm($L['my_choose_reply']);
		$content = '';
		if($reason == $L['my_manage_reason']) $reason = '';
		$reason = dhtmlspecialchars($reason);
		$message = isset($message) ? 1 : 0;
...
if($action) exit(include template('my_club_manage', 'mobile'));
?>