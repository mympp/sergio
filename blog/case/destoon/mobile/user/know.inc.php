...
if($action == 'answer') {
	...
	$linkurl = mobileurl($mid, 0, $itemid);
	/*
	...
	*/
	...
	if(isset($_POST['ok'])) {
		...
		if(!$content) dtexit($L['type_answer']);
		...
		dtexit('ok',$linkurl,$status == 3 ? $L['answer_success'] : $L['answer_check']);
		dtexit($status == 3 ? $L['answer_success'] : $L['answer_check'], '', 'parent.window.location="'.$linkurl.'";');
	} else {
		$head_name = $L['answer_title'];
		include template('my_'.$module, 'mobile');
	}
	exit;
}
$MG['know_limit'] > -1 or dlayerm(lang('message->without_permission_and_upgrade'), 'goback');
...
switch($action) {
	case 'add':
		...
		if($MG['know_limit'] && $limit_used >= $MG['know_limit']) dlayerm(lang($L['info_limit'], array($MG[$MOD['module'].'_limit'], $limit_used)), $_userid ? $MODULE[2]['linkurl'].$DT['file_my'].'?mid='.$mid : $MODULE[2]['linkurl'].$DT['file_my']);
		if($MG['day_limit']) {
			$today = $today_endtime - 86400;
			$r = $db->get_one("SELECT COUNT(*) AS num FROM {$table} WHERE $sql AND addtime>$today");
			if($r && $r['num'] >= $MG['day_limit']) dlayerm(lang($L['day_limit'], array($MG['day_limit'])), $_userid ? $MODULE[2]['linkurl'].$DT['file_my'].'?mid='.$mid : $MODULE[2]['linkurl'].$DT['file_my']);
		}
		...
		$thumbs= explode(",", $thumbs);
		 ...
		 $post['thumb2'] = $thumbs[2];
			...
			if($post['credit'] && $post['credit'] > $_credit) dtexit($L['balance_lack']);
			if(isset($post['hidden']) && $post['credit'] + $MOD['credit_hidden'] > $_credit) dtexit($L['balance_lack']);
