...
if(in_array($job, array('group', 'reply', 'join', 'fans', 'manage'))) {
	require DT_ROOT.'/mobile/user/club_'.$job.'.inc.php';
} else {
	$job = '';
	$MG['club_limit'] > -1 or dlayerm(lang('message->without_permission_and_upgrade'), 'goback');
	...
	switch($action) {
		case 'add':
		    $head_name = $L['post_title'];
		    $back_link = $MYURL.'?action=info';
			if($gid) $GRP = get_group($gid);
			if($MG['club_limit'] && $limit_used >= $MG['club_limit']) dlayerm(lang($L['info_limit'], array($MG[$MOD['module'].'_limit'], $limit_used)),'goback');
		...
		$thumbs= explode(",", $thumbs);
		 ...
		 $post['thumb2'] = $thumbs[2];
			foreach($post as $k=>$v) {
			  ...
			  $gid or dtexit($L['my_choose_group'], $MOD['linkurl']);
			  $GRP = get_group($gid);
			  ($GRP && $GRP['status'] == 3) or dtexit($L['my_not_group']);
...
if($job){
include template('my_club_'.$job, 'mobile');
}else{
