...
require '../glbackurl.inc.php';
//echo $pay_indexurl;die;
$charge_title = '';
if($action == 'ajax') {
	$itemid or exit('ko');
} else {
	$auth = isset($auth) ? decrypt($auth, DT_KEY.'QRPAY') : '';
	$auth or dheader($pay_recordurl);
	$t = explode('|', $auth);
	$itemid = $orderid = intval($t[0]);
	($itemid && $t[2] == $DT_IP) or dheader($pay_recordurl);
	$charge_title = $t[1];
}
...
if(!$r || !$_username || $r['username'] != $_username || $r['status'] != 0 || $r['bank'] != 'weixin') dheader($pay_recordurl);
$bank = 'weixin';
$PAY = cache_read('pay.php');
$PAY[$bank]['enable'] or dheader($pay_recordurl);
...
$charge_title='认证会员缴费(流水号:'.$orderid.')';
...
if(strpos($rec, 'code_url') !== false) {
	...
	if(strpos($rec, 'return_msg') !== false) {
		if(function_exists('libxml_disable_entity_loader')) libxml_disable_entity_loader(true);
		$x = simplexml_load_string($rec, 'SimpleXMLElement', LIBXML_NOCDATA);
		dalert(convert($x->return_msg, 'UTF-8', DT_CHARSET), $pay_recordurl);
	} else {
		dalert('Can Not Connect weixin', $pay_recordurl);
	}
}
$pay_indexurl=DT_PATH."api/mypay_notarize.php?action=notarize&itemid=$itemid";
//$pay_recordurl=DT_PATH."javascript:history.back(-1)";
$pay_recordurl=DT_PATH."api/mypay_notarize.php?action=cancel&itemid=$itemid";
?>
<html>
<head>
...
<a href="<?php echo $pay_indexurl;?>" style="color:#2E7DC6;text-decoration:none;">已经支付</a>
&nbsp;&nbsp;
<a href="<?php echo $pay_recordurl;?>" style="color:#2E7DC6;text-decoration:none;">取消支付</a>
</div> 
	</div>
		...
		function() {
			$.get('?action=ajax&itemid=<?php echo $itemid;?>', function(data) {
				if(data == 'ok') {
					...
					window.location.href = '<?php echo $pay_indexurl;?>';
				}
			});
		}, 
	3000);
