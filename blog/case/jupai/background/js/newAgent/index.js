...
passenger.renderNewDistributeChart = function() {
    ...
    .done(function(data) {
        if(data.success) {
            ...
            var chipArr = ["e4:95:6e:4d:51:7a","e4:95:6e:4d:50:41"];
            if($.inArray(chipNo, chipArr) >= 0) {
                data.result.topPoiList.unshift({label: "重汽翡翠清河", value: "75.63"}, {label: "汇源华庭小区", value: "8.31"},{label: "景苑花园", value: "7.57"}, {label: "齐鲁软件园", value: "5.24"}, {label: "其他", value: "3.25"});
                ...
                for (var i = 0; i < tempDataSet.length; i++) {
                    ...
                    for(var j = 0; j < 50; j++) {
                        var newData = [];
                        newData[0] = parseFloat(dataArr[0]) + (Math.random()-0.5) * 0.005;
                        newData[1] = parseFloat(dataArr[1]) + (Math.random()-0.5) * 0.005;
                        newData[2] = dataArr[2];
                        console.log(newData);
...
passenger.renderPageData = function() {
    ...
    //new
    $.ajax({
        url: BASEURL + 'background/router/getCliAnalysis.do',
        type: 'POST',
        data: {
            chip_no: chipNo,
            scene_name: sceneName,
    ...
    .done(function(data) {
        if(data.success) {
            ...
            var chipArr = ["e4:95:6e:4d:51:7a","e4:95:6e:4d:50:41"];
            if($.inArray(chipNo, chipArr) >= 0) {
                data.result.outdoor = data.result.outdoor * 10;
                data.result.indoor = data.result.indoor * 10;
                data.result.old = data.result.old * 10;
                data.result.new = data.result.new * 10;
                for (var i = 0; i < data.result.frequency.length; i++) {
                    data.result.frequency[i] = data.result.frequency[i] * 10;
                }
                for (var i = 0; i < data.result.period.length; i++) {
                    data.result.period[i] = data.result.period[i] * 10;
                }
                for (var i = 0; i < data.result.period_in.length; i++) {
                    data.result.period_in[i] = data.result.period_in[i] * 10;
                }
                for (var i = 0; i < data.result.remain_time.length; i++) {
                    data.result.remain_time[i] = data.result.remain_time[i] * 10;
                }
            }
        ...
        if(data.success) {
            if(data.result.messageList.length > 0){
                ...
                $("#advert-list-table .edit-btn").on("click", function() {
                    if(index.adType == 0) {
                        window.open(BASEURL + 'editAdvert/adid/' + $(this).attr('data-mid'));
                    } else {
...
childAccount.init = function(){
    ...
    function getSubAdvertNum(rechargeMoney, subPrice){
        var sub_price = Math.floor((subPrice == "" ? 0 : subPrice));
        if(sub_price == 0){
            return 0;
        }else{
            return (Math.floor(rechargeMoney) / sub_price);
        }
    }
    $('input[name="recharge-type"]').on('change', function() {
        ...
        var advert_num = 0;
        if(ad_type == "0"){
            advert_num = Math.floor(getSubAdvertNum(rechargeMoney, $("#sub-perprice").val()));
            $("#sub-get-num").text(advert_num);
            $("#cost-money").text(getSubAdvertNum(rechargeMoney, $("#sub-perprice").val()) * parseFloat($("#self-perprice").text()));
        }else if(ad_type == "2"){
            advert_num = Math.floor(getSubAdvertNum(rechargeMoney, $("#sub-notice-perprice").val()));
            $("#sub-get-num").text(advert_num);
            $("#cost-money").text(getSubAdvertNum(rechargeMoney, $("#sub-notice-perprice").val()) * parseFloat($("#self-notice-perprice").text()));
        }
    });
...
applyVipPortrait.externalChannelsInfo = function(){
    $.ajax({
        url: BASEURL + 'message/externalChannelsInfo',
        type: 'POST',
        dataType: 'json',
        data: {
