	...
	$.fn.extend({
				takungaeImgup : function(opt, serverCallBack) {
					...
					$(this).change(function() {
						...
						if (fileList.length > imageNum|| totalNum > imageNum) {
							...
							for ( var i = 0; i < fileList.length; i++) {
								...
								$img0.attr("src","../../Public/Home/1.6/Images/a7.png").appendTo($section);
					...
					$(".wsdel-ok").click(function(event) {
						...
						var img = $(delParent).find("input[name=file_name]").val();
						var url = pc_img_del
						var data = {'img':img};
						...
						$.ajax({
							...
							success : function(data) {
								...
								if (data.error == 0) {
									...
									var htmlStr = "<input type='text' style='display:none;' name='"+ opt.formData.name+ "' value='"+ data.url+ "'>";
									obj.append(htmlStr);
									// if (successCallBack) {
