		...
		$('#submit-button')
			.click(function() {
				...
				if (parseInt($(this).attr('step')) === 1) {
					...
					var url = API_URL + '/router/addRouter.do';
		...
		$("#scenes-list-table").on("click", ".scene-detail", function() {
			...
			if ($(this).attr("data-status") == "0") {
				layer.confirm("为了便于数据分析，建议您完善“" + decodeURIComponent(current_name) + "”场景信息", {
					...
					window.open(BASEURL + 'background/box2.html?chip_id=' + chip_id + '&chip_no=' + chip_No + '&scene_name=' + init_name);
				}, function() {
