(function($) {
  var pluginName = 'particleground';//插件名称
  function Plugin(element, options) {
    ...
    var orientationSupport = !!window.DeviceOrientationEvent;//3d重力
    var tiltX = 0;
    var tiltY = 0;
    var paused = false;
    ...
    function init() {
      ...
      if($("body").attr("pagename")=='homePage'){
            $("#cav").css({"position":"fixed","top":"81"})
        };
      $el.prepend($canvas);
      canvas = $canvas[0];
     ...
     * Style the canvas
     */
    function styleCanvas() {//设置canvas样式的函数
        
        if($("body").attr("pagename")=='homePage'){
            var h = $(window).height();
            var w = $(window).width();
        }else{
            var w = $("#main-box").width();
            var h = $("#main-box").height();
        }
        if($("body").attr("pagename")=='newsList'){
            h = $(document).height()-$("footer").height()-$(".nav").height()-$("#nav-wrap").height()-40;
        }
        canvas.width = w;
        canvas.height = h;
